...
public class DynamicLoadingTest extends BaseTest {
    ...
    public void tc02_go_to_DynamicLoading_Main_page(){
        ...
        expectedHeading = Utils.readProperty("dlmpHeading");
        wp.goToPage(pageName);
        dlmp = new DynamicLoadingMainPage(driver);
        Assert.assertTrue(dlmp.isPageDisplayed(dlmp.getPageHeading(),expectedHeading),pageName + " page is not loaded");
    }

    @Test(dependsOnMethods = "tc02_go_to_DynamicLoading_Main_page")
    public void tc03_go_to_DynamicLoading_Ex2_page(){
        dlmp = new DynamicLoadingMainPage(driver);
        expectedHeading = Utils.readProperty("dl2pHeading");
        dlmp.goToPage(expectedHeading);
        dl2p = new DynamicLoadingEx2Page(driver);
        Assert.assertTrue(dl2p.isPageDisplayed(dl2p.getPageHeading(),expectedHeading),pageName + " page is not loaded");
    }
